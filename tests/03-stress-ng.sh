...
stress-ng --cpu 0 --cpu-method all --verify --metrics --timeout "${half}m" >> "$LOG" 2>&1
tk_mark_progress "$RUN_DIR" "stress-ng single-core"
stress-ng --cpu 2 --cpu-method all --verify --metrics --timeout "${half}m" >> "$LOG" 2>&1
tk_scan_log stress-ng "$LOG" >> "$LOG"; exit $?
